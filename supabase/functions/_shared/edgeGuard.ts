...
export async function requireAuth(req: Request): Promise<GuardContext> {
  ...
  if (ctx.user.id.startsWith('anon:')) throw new Error('Unauthorized');
  return ctx;
}

/** Accept signed-in users or anonymous callers using the public anon key (IP rate-limited). */
export async function resolveAuthContext(req: Request): Promise<GuardContext> {
