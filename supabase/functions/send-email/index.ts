...
// Generic outbound email via SendGrid.
//
// Secrets:
// - SUPABASE_URL
// - SUPABASE_ANON_KEY
...
Deno.serve(async (req) => {
  ...
  const payload = {
    ...
    ...(body.replyTo?.email ? { reply_to: { email: body.replyTo.email, name: body.replyTo.name || undefined } } : {}),
    subject,
    content,
  };
  ...
  try {
    ...
    await logEdgeEvent({
      ...
      level: 'error',
      event: 'send_failed',
      meta: { userId: ctx.user.id, ip: ctx.ip, toEmail, subject: subject.slice(0, 120), error: (e as Error)?.message || String(e) },
    });
