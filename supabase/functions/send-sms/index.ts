...
// Generic outbound SMS via Twilio or REST API (API_ID + API_KEY).
//
// Secrets (Twilio):
// - TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_PHONE (optional)
//
// Secrets (REST SMS — e.g. VatanSMS-compatible gateways):
// - SMS_API_ID (or API_ID), SMS_API_KEY (or API_KEY), SMS_SENDER_ID
// - SMS_PROVIDER=rest (optional; auto-detected when Twilio is absent)
...
Deno.serve(async (req) => {
  ...
  const rlUser = await rateLimit({ key: `send-sms:user:${ctx.user.id}`, limit: 10, windowSeconds: 60 });
  const rlIp = await rateLimit({ key: `send-sms:ip:${ctx.ip}`, limit: 30, windowSeconds: 60 });
  if (!rlUser.ok || !rlIp.ok) {
    await logEdgeEvent({ namespace: 'send-sms', level: 'warn', event: 'rate_limited', meta: { ip: ctx.ip, userId: ctx.user.id } });
    return json(
  ...
  if (body.idempotencyKey) {
    const ok = await requireIdempotency({ namespace: 'send-sms', key: `${ctx.user.id}:${body.idempotencyKey}` });
