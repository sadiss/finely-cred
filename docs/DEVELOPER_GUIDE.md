...
`src/components/chat/PublicChatWidget.tsx` is the public-site chat bubble. **One on-shift face everywhere** — launcher, header, lane pickers, typing indicator, and every assistant bubble all resolve through `resolveChatStaffPresentation()` in `src/lib/chatStaffPresentation.ts` (wrappers in `src/components/chat/publicChatPersonaUi.ts`). That function reads `resolveStaffOnDuty()` / `resolveStaffOnDutyForLane()` from `src/data/staffRoster.ts`, applies the persona's presentation styling from `PERSONA_PRESENTATION_STYLES` in `src/lib/chatPersonaStyles.ts`, and falls back to the **first active roster member** — never a synthetic guide.

**AI disclosure (compliance):** the on-shift portrait, first name, and role are shown, but assistant bubbles carry a persistent **"AI assist"** badge and the opening welcome includes: *"You're chatting with Finely's AI, standing in for {Name}, {role} — they'll take over live when you're ready."* `buildAiAssistSystemPrompt()` instructs the model to speak in that person's voice and role while explicitly stating it is AI assisting on their behalf — **do not pretend to be human**. Portal coach (`HubAiCoachPanel.tsx`) uses the same resolver and disclosure pattern.

`PERSONA_PRESENTATION_STYLES` in `chatPersonaStyles.ts` is a per-`AgentPersonaId` record (Morgan/finely_advisor, Taylor/dispute_coach, Marcus/funding_strategist, Casey/debt_strategist, Ruth/finely_coowner, etc.) — each entry supplies a gradient, avatar fallback, welcome line, and chip styling layered onto whichever roster member is on shift.
