    ...
    >
      {debugUi ? (
        <div
          ...
          className="fixed top-24 left-5 lg:left-5 lg:top-24 lg:w-[340px] z-[220] w-[320px] max-w-[calc(100vw-40px)] rounded-2xl border border-white/10 bg-black/80 backdrop-blur-xl p-4 text-white/80 shadow-2xl"
        >
          <div className="text-[10px] uppercase tracking-widest text-white/50 font-mono">debugUi links</div>
          <div className="mt-2 grid gap-2 text-xs">
            <a
              className="inline-flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 hover:bg-white/[0.06] transition-all"
            ...
            <button
              type="button"
              className="inline-flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 hover:bg-white/[0.06] transition-all text-left"
            ...
            {nestedScrollSuspects.length === 0 ? (
              ...
              <div className="mt-3 space-y-2">
                {nestedScrollSuspects.slice(0, 8).map((s, i) => (
                  <div key={`${s.tag}_${i}`} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                    <div className="flex items-center justify-between gap-3">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute inset-0 world-map-motif" />
        <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-black/35" />
        {/* premium brand glow */}
        <div
          className="absolute -top-40 left-1/2 -translate-x-1/2 w-[1200px] h-[560px] blur-3xl opacity-30"
          style={{
            background:
        ...
        {isAdmin ? (
          <div className="grid lg:grid-cols-[340px_1fr] gap-8">
            ...
            <div className="min-w-0 flex flex-col">
              ...
              <div className="pb-16">
                <div data-fc-route-content="1" data-fc-route-pathname={pathname}>
                  {children}
                </div>
              </div>
            </div>
