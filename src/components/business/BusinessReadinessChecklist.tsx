    ...
    <div className={`${finelyOsCatalogCard('violet')} !p-5 space-y-4`}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className={`mt-1 ${FINELY_OS_ENTITY_BODY}`}>Keep this tight and sequential; avoid premature applications.</div>
        </div>
        <div className="min-w-[220px]">
          <div className={`flex items-center justify-between ${FINELY_OS_ENTITY_SUBLABEL}`}>
            <span>Progress</span>
            <span className={`${FINELY_OS_ENTITY_VALUE} font-mono`}>{progress.pct}%</span>
          </div>
      ...
      <div className={`grid gap-4 ${compact ? 'md:grid-cols-2' : 'md:grid-cols-2 lg:grid-cols-4'}`}>
        {grouped.map(([cat, items]) => (
          <div key={cat} className={`${finelyOsCatalogCard('sky')} !p-4 fc-surface-harmony space-y-3`}>
            <div className="text-[10px] uppercase tracking-widest text-amber-300 font-bold">{cat}</div>
                ...
                return (
                  <div key={it.id} className="flex items-start gap-2">
                    <button
                      ...
                      className="mt-0.5 text-white/45 hover:text-white transition-colors"
                      title={done ? 'Mark incomplete' : 'Mark complete'}
                    >
                      {done ? <CheckCircle2 size={16} className="text-emerald-400" /> : <Circle size={16} />}
                    </button>
                    <div className="min-w-0">
                      <div className={`text-sm ${FINELY_OS_ENTITY_VALUE}`}>{it.label}</div>
                      {!compact && <div className={`${FINELY_OS_ENTITY_SUBLABEL} text-[11px]`}>{it.desc}</div>}
                      {(it.path || it.externalHref) && (
                        <button
                          type="button"
                          onClick={() => open(it)}
                          className="mt-1 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-emerald-300 hover:text-emerald-200 transition-colors"
                        >
