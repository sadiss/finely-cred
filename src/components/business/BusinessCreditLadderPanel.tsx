...
export function BusinessCreditLadderPanel({ partnerId }: { partnerId: string }) {
  ...
  return (
    <div className={`${finelyOsGlassShell('panel', 'amber')} space-y-5`}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className={`mt-2 text-2xl font-light ${FINELY_OS_ENTITY_VALUE}`}>Business Credit Ladder</div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        {BUSINESS_LADDER_TASKS.map((s) => {
          ...
          const done = st.done;
          return (
            <div key={s.step} className={`${finelyOsInlineListItem()} space-y-3`}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className={FINELY_OS_ENTITY_VALUE}>{s.title}</div>
                  <div className={`mt-1 ${FINELY_OS_ENTITY_BODY}`}>{s.notes}</div>
                </div>
      ...
      <div className={`text-[11px] ${FINELY_OS_ENTITY_SUBLABEL}`}>
        Tip: this ladder seeds business-scoped tasks automatically. Track execution in <span className={`${FINELY_OS_ENTITY_VALUE} font-mono`}>/portal/projects</span>.
      </div>
    </div>
  );
}
