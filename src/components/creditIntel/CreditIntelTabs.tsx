  ...
  return (
    <div className="overflow-x-auto">
      ...
      {args.rows.length > rows.length && (
        <div className="mt-3 text-[11px] text-white/45">Showing first {rows.length} rows.</div>
      )}
    </div>
  );
}

/** Render table rows as one card per row — paginated catalog for Collections/Inquiries. */
function sectionTableAsCards(args: { columns: string[]; rows: string[][] }) {
  const { columns, rows } = args;
  return (
    <FinelyOsPaginatedStack
      items={rows}
      pageSize={INTEL_CATALOG_PAGE_SIZE}
      itemSpacingClassName="grid gap-3 md:grid-cols-2"
      emptyMessage="No rows in this section."
      renderItem={(row, ri) => (
        <div key={ri} className="fc-soft-surface p-4 space-y-2">
          {columns.map((col, ci) => {
            const val = row[ci] ?? '';
            if (!val) return null;
            return (
...
/** Phase 1: Render structured section items (collections/inquiries) as paginated cards. */
function sectionItemsAsCards(items: ParsedSectionItem[]) {
  return (
    <FinelyOsPaginatedStack
      items={items}
      pageSize={INTEL_CATALOG_PAGE_SIZE}
      ...
      renderItem={(item, i) => (
        <div key={item.rowIndex ?? i} className="fc-soft-surface p-4 space-y-2">
...
export function CreditIntelTabs({
  ...
  const openTasks = () => {
    ...
    if (location.pathname.startsWith('/portal')) return navigate('/portal/projects');
  };
  ...
  const disputeReasonsByCandidateId = useMemo(() => {
    ...
    for (const c of disputeCandidates) {
      m.set(c.id, buildFactualDisputeSuggestions({ parsed: safeParsed, candidate: c }));
    }
    return m;
  ...
  const renderSection = (key: string) => {
                ...
                return (
                  <div key={item.rowIndex ?? i}>
                    <div
                      ...
                      className="fc-soft-surface p-4 space-y-2"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <span className="text-[10px] uppercase tracking-widest text-white/45 shrink-0">{itemLabel}</span>
                        <button
                          type="button"
                          onClick={() =>
                          ...
                          className="inline-flex items-center gap-1.5 px-2 py-1.5 rounded-lg fc-light-chrome-btn text-[10px] font-bold uppercase tracking-wider text-white/70 disabled:opacity-60"
              ...
              renderItem={(row, i) => {
                ...
                return (
                  <div key={i}>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <span className="text-[10px] uppercase tracking-widest text-white/45 shrink-0 truncate">{itemLabel}</span>
                        <button
                          type="button"
                          onClick={() =>
                            void captureSectionItemScreenshot({
    ...
    return (
      <div
        ...
        className="fc-soft-surface p-5 space-y-3"
      >
        ...
        {looksLikeBureauMatrixTable && s.table?.rows?.length ? (
          <details className="fc-soft-surface backdrop-blur-sm p-4 platinum-shimmer fc-platinum-frame">
            <summary className="cursor-pointer select-none flex items-center justify-between gap-3 text-[10px] font-black uppercase tracking-widest text-white/60">
              ...
              <span className="text-white/40 font-mono">{s.table.rows.length} rows</span>
            </summary>
          ...
          ['collections', 'inquiries'].includes(key)
            ? sectionTableAsCards({ columns: s.table!.columns, rows: s.table!.rows })
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-visible" data-fc-credit-intel="1">
      ...
      {restoreModeOn ? (
        <div className={`${FINELY_OS_NOTICE_INFO} space-y-4`}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 fc-light-chrome-btn text-[10px] font-black uppercase tracking-widest text-white/70 transition-all"
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="fc-soft-surface-lg p-5 space-y-2">
              ...
              <div className="pt-2 flex flex-wrap gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-2 fc-light-chrome-btn text-[10px] font-black uppercase tracking-widest text-white/70 transition-all"
                >
            ...
            <div className="fc-soft-surface-lg p-5 space-y-2">
              ...
              {nextMissingEvidenceCandidate ? (
                ...
                <>
                  <div className="text-white/90 font-semibold">Evidence looks good</div>
        ...
        <div className="flex justify-end">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 fc-light-chrome-btn text-[10px] font-black uppercase tracking-widest text-white/60 transition-all"
          >
            Show Restore Mode
          </button>
        </div>
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
            <p className="text-[10px] uppercase tracking-widest text-white/45">3-bureau overview (derived from extracted data)</p>
          ...
          <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
            <button
              type="button"
              className="flex items-center justify-between w-full text-left px-4 py-2 fc-light-chrome-btn text-[10px] font-black uppercase tracking-widest text-white/45"
            ...
            {showDiagnostics && (
              <>
                ...
                <div className="flex justify-end">
                  <button
                    type="button"
                    className="px-4 py-2 fc-light-chrome-btn text-[10px] font-black uppercase tracking-widest text-white/70"
                    onClick={async () => {
                      try {
                ...
                <pre className="fc-soft-surface p-4 text-[11px] text-white/70 whitespace-pre-wrap overflow-auto max-h-[280px]">
                  {diagnosticsText}
                </pre>
              </>
            )}
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
              <p className="text-[10px] uppercase tracking-widest text-white/45">Scores (best-effort extraction)</p>
                      ...
                      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest ${
                        m.family === 'FICO'
                          ...
                          : m.family === 'VantageScore'
                            ...
                            : 'border-white/[0.08] fc-soft-surface text-white/70'
                      }`}
              ...
              {commonScoreRows.length ? (
                <div className="fc-soft-surface p-4 space-y-2">
                  <p className="text-[10px] uppercase tracking-widest text-white/45">Common score models matrix</p>
                  <p className="text-white/60 text-sm">
              ...
              {safeParsed.scores?.length ? (
                <div className="fc-soft-surface p-4 text-[11px] text-white/60">
            ...
            <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
              <p className="text-[10px] uppercase tracking-widest text-white/45">Score models (what they are + who uses them)</p>
              <div className="space-y-3 text-white/70 text-sm">
                <div className="fc-soft-surface p-4">
                  <div className="text-white/90 font-semibold">FICO 8</div>
                  <div className="mt-1 text-white/60">
                    Most commonly used “general lending” model for credit cards/personal loans (varies by lender).
                  </div>
                </div>
                <div className="fc-soft-surface p-4">
                  <div className="text-white/90 font-semibold">Mortgage classic scores</div>
                  <div className="mt-1 text-white/60">
                ...
                <div className="fc-soft-surface p-4">
                  <div className="text-white/90 font-semibold">VantageScore (3.0 / 4.0)</div>
                  <div className="mt-1 text-white/60">
      ...
      {tab === 'pi' && (
        <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
          {(() => {
            ...
            return (
              <div
                className="fc-soft-surface-lg backdrop-blur-xl overflow-hidden relative platinum-shimmer fc-platinum-frame"
              >
                <button
                  type="button"
                  onClick={() => setPiOpen((v) => !v)}
                  className="w-full flex items-center justify-between gap-6 px-6 py-5 hover:bg-white/[0.04] transition-colors"
                >
                  ...
                  <ChevronDown size={18} className={`text-white/45 transition-transform ${piOpen ? 'rotate-180' : ''}`} />
                </button>
      ...
      {tab === 'public_records' && (
        <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-6">
          <p className="text-[10px] uppercase tracking-widest text-white/45">Public records</p>
          <p className="text-white/60 text-sm">
          ...
          {hasPublicRecords ? (
            <>
              {renderSection('public_records')}
              <div className="fc-soft-surface p-4 text-white/60 text-sm">
            ...
            <div className="fc-soft-surface-lg p-5">
              <div className="text-white/90 font-semibold">No public records found</div>
          ...
          <div className="fc-soft-surface-lg p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-white/90 font-semibold">Public record dispute candidates</div>
              <button
                type="button"
      ...
      {tab === 'negatives' && (
        <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-8">
          <p className="text-[10px] uppercase tracking-widest text-white/45">Disputable negatives (categorized)</p>
          <p className="text-white/60 text-sm">
          ...
          <div className="fc-soft-surface-lg p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-white/90 font-semibold">Negatives breakdown (from dispute engine)</div>
              <button
                type="button"
                onClick={() => (onOpenLetterGenerator ? onOpenLetterGenerator() : setTab('disputes'))}
                className="px-4 py-2 rounded-xl bg-amber-500 text-black font-black uppercase tracking-widest text-[10px] hover:brightness-110 transition-all"
            ...
            <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {(() => {
                ...
                return rows.map((r) => (
                  <div key={r.k} className="fc-light-glass-panel fc-light-chrome-panel p-4">
                    <div className="text-[10px] uppercase tracking-widest text-white/45">{r.label}</div>
                    <div className="mt-2 text-2xl font-light text-white/90">{counts.get(r.k) ?? 0}</div>
                  </div>
                ));
              })()}
            </div>
          ...
          {candidatePriority.length > 0 && (
            <div className="fc-soft-surface-lg p-5 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-white/90 font-semibold">Top priority disputes (ranked)</div>
                <div className="text-[10px] uppercase tracking-widest text-white/45 font-mono">severity 1–100</div>
              </div>
                ...
                {candidatePriority.slice(0, 8).map((c) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="text-white/90 font-semibold truncate">{c.account}</div>
                        <div className="mt-1 text-[10px] uppercase tracking-widest text-white/45 font-mono">
                          {bureauShortCode(c.bureau)} • {c.type}
                        </div>
                      </div>
                      <div className="shrink-0 inline-flex items-center px-2 py-1 rounded-full border border-fuchsia-500/30 bg-fuchsia-500/10 text-fuchsia-200 text-[10px] font-black uppercase tracking-widest">
                        {c.severity}
                      </div>
                    </div>
                    <div className="mt-2 text-white/60 text-sm space-y-1">
                      {(c.insight?.whyTop ?? []).slice(0, 3).map((w, i) => (
                        <div key={i} className="text-[11px] text-white/90/65">• {w}</div>
                      ))}
                    </div>
                    <div className="mt-3 flex justify-end">
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-3 py-2 fc-light-chrome-btn text-[10px] font-black uppercase tracking-widest text-white/90/75 transition-all"
                        title="Open insights"
                      >
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div className="fc-soft-surface-lg p-5">
              <div className="text-[10px] uppercase tracking-widest text-white/45">Late payments</div>
              <div className="mt-2 text-3xl font-light text-white/90">{latePaymentTradelines.length}</div>
              <button
                type="button"
                onClick={() => setScrollToAccount(latePaymentTradelines[0]?.creditorName ?? null)}
                className="mt-3 px-4 py-2 fc-light-chrome-btn text-[10px] font-black uppercase tracking-widest text-white/70 transition-all"
            ...
            <div className="fc-soft-surface-lg p-5">
              <div className="text-[10px] uppercase tracking-widest text-white/45">Public records</div>
              <div className="mt-2 text-3xl font-light text-white/90">{hasPublicRecords ? publicRecordsCount : 0}</div>
          ...
          {renderSection('public_records') && (
            <div className="space-y-4">
              <div className="text-white/90 font-semibold">Public records (from report sections)</div>
              {renderSection('public_records')}
            </div>
          )}
        </div>
      ...
      {tab === 'inquiries' && (
        <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
          <p className="text-[10px] uppercase tracking-widest text-white/45">Inquiries</p>
      ...
      {tab === 'strategy' && (
        <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.06] p-5 space-y-2">
              ...
              {scoreFactors.helping.length ? (
                ...
                <p className="text-white/45 text-sm">No positive factors detected. Add on-time accounts and keep utilization low to build score.</p>
              )}
            </div>
            <div className="rounded-2xl border border-fuchsia-500/15 bg-fuchsia-500/[0.06] p-5 space-y-2">
              ...
              {scoreFactors.hurting.length ? (
                ...
                <p className="text-white/45 text-sm">No major negative factors detected from the extracted data.</p>
              )}
            </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="fc-soft-surface-lg p-5 space-y-3">
              <div className="text-white/90 font-semibold">Next best actions</div>
            ...
            <div className="fc-soft-surface-lg p-5 space-y-3">
              <div className="text-white/90 font-semibold">What we extracted</div>
              ...
              <div className="fc-light-glass-panel fc-light-chrome-panel p-4 text-[11px] text-white/60">
            ...
            {(() => {
              ...
              return (
                <>
                  <div className="fc-soft-surface-lg p-5 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-white/90 font-semibold">Strategy timeline</div>
                      <div className="text-[10px] uppercase tracking-widest text-white/45 font-mono">action order</div>
                    </div>
                    <div className="space-y-3">
                      {timeline.map((s, idx) => {
                        const isNext = !s.done && timeline.slice(0, idx).every((x) => x.done);
                        const tone = s.done ? 'border-emerald-500/25 bg-emerald-500/10' : isNext ? 'border-fuchsia-500/25 bg-fuchsia-500/10' : 'border-white/[0.08] fc-soft-surface';
                        const label = s.done ? 'done' : isNext ? 'next' : 'later';
                        return (
                          <div key={s.id} className={`rounded-2xl border p-4 ${tone}`}>
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="text-white/90/90 font-semibold">{idx + 1}. {s.title}</div>
                                <div className="mt-1 text-white/60 text-sm">{s.why}</div>
                              </div>
                              <span className="shrink-0 inline-flex items-center px-2 py-1 rounded-full fc-soft-surface text-[10px] font-black uppercase tracking-widest text-white/70">
                                {label}
                              </span>
                            </div>
                            {s.cta ? (
                  ...
                  <div className="fc-soft-surface-lg p-5 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-white/90 font-semibold">Litigation readiness (signals)</div>
                      <div className="text-[10px] uppercase tracking-widest text-white/45 font-mono">derived</div>
                    </div>
                    <div className="grid sm:grid-cols-3 gap-3">
                      <div className="fc-light-glass-panel fc-light-chrome-panel p-4">
                        <div className="text-[10px] uppercase tracking-widest text-white/45">Evidence</div>
                      ...
                      <div className="fc-light-glass-panel fc-light-chrome-panel p-4">
                        ...
                        <div className="mt-2 text-white/90 text-lg font-semibold">{contradictionsTotal}</div>
                        <div className="mt-1 text-[11px] text-white/90/55">Cross-bureau mismatches found</div>
                      </div>
                      <div className="fc-light-glass-panel fc-light-chrome-panel p-4">
                        <div className="text-[10px] uppercase tracking-widest text-white/45">High severity</div>
                        <div className="mt-2 text-white/90 text-lg font-semibold">{highSeverity}</div>
                        <div className="mt-1 text-[11px] text-white/90/55">Items severity ≥ 80</div>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2 pt-2">
                      <button
          ...
          {/* Suggested dispute angles from parsed data */}
          {disputeCandidates.length > 0 && (
            <div className="fc-soft-surface-lg p-5 space-y-3">
              <div className="text-white/90 font-semibold">Suggested dispute angles</div>
              ...
              <div className="space-y-2">
                {disputeCandidates.slice(0, 5).map((c) => {
                  ...
                  return (
                    <div key={c.id} className="fc-soft-surface bg-white/[0.06] overflow-hidden">
                      <button
                        type="button"
                      ...
                      >
                        <span className="text-white/90/90 font-medium truncate">{c.account}</span>
                        <span className="text-[10px] uppercase text-white/45 shrink-0">{bureauShortCode(c.bureau)} · {c.type}</span>
                        {isExpanded ? <ChevronDown size={14} className="text-white/45" /> : <ChevronRight size={14} className="text-white/45" />}
                      </button>
                      {isExpanded && reasons.length > 0 && (
                        <div className="px-4 pb-4 space-y-2">
                          {reasons.map((r) => (
                            <div key={r.id} className="text-white/70 text-sm pl-2 border-l-2 border-white/20">{r.text}</div>
                          ))}
                          <button
                            type="button"
              ...
              {disputeCandidates.length > 5 && (
                <p className="text-[11px] text-white/45">Showing first 5 of {disputeCandidates.length}. Open Disputes tab for all with suggested reasons.</p>
              )}
            </div>
          )}
      ...
      {tab === 'disputes' && (
        <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
          <p className="text-[10px] uppercase tracking-widest text-white/45">Dispute letters — one place</p>
          <p className="text-white/70 text-sm">
            The same items you see in <strong className="text-white/90/90">Collections</strong> and <strong className="text-white/90/90">Inquiries</strong> are used for dispute letters. Select items and generate your letter in the <strong className="text-fuchsia-400">Disputes</strong> tab in the toolbar above — no duplicate selection.
          </p>
          {disputeCandidates.length === 0 ? (
            <div className="text-white/60 text-sm">
            ...
            <>
              <div className="rounded-xl border border-fuchsia-500/20 bg-fuchsia-500/10 p-4">
                <p className="text-white/90/90 text-sm font-medium">{disputeCandidates.length} negative item{disputeCandidates.length !== 1 ? 's' : ''} detected</p>
                {onOpenLetterGenerator && (
                  <>
                      ...
                      <>
                        <div className="grid md:grid-cols-4 gap-3">
                          <div className="fc-soft-surface-lg p-4">
                            <div className="text-[10px] uppercase tracking-widest text-white/45">Readiness</div>
                            <div className="mt-2 text-2xl font-light text-white/90">{intelReadiness.score}/100</div>
                            <div className="mt-2 text-[11px] text-white/90/55">Derived from extraction + identity + evidence.</div>
                          </div>
                          <div className="fc-soft-surface-lg p-4">
                            <div className="text-[10px] uppercase tracking-widest text-white/45">Negatives</div>
                            <div className="mt-2 text-2xl font-light text-white/90">{restoreDisputeList.length}</div>
                            <div className="mt-2 text-[11px] text-white/90/55">Items available to dispute in this report.</div>
                          </div>
                          <div className="fc-soft-surface-lg p-4">
                            <div className="text-[10px] uppercase tracking-widest text-white/45">Evidence coverage</div>
                        ...
                        <div className="space-y-3">
                          {entries.map(([type, list]) => {
                            ...
                            return (
                              <details key={type} className="group fc-soft-surface-lg p-4" open={openByDefault}>
                                ...
                                <div className="mt-4 grid md:grid-cols-2 gap-3">
                                  {list.map((c) => {
                                    ...
                                    return (
                                      <div
                                        key={c.id}
                                        className="fc-light-glass-panel fc-light-chrome-panel p-4 flex items-start justify-between gap-3"
                                      >
                                        <div className="min-w-0">
                                          <p className="text-white/90 font-semibold truncate">{c.account}</p>
                                          <p className="mt-1 text-[10px] uppercase tracking-widest text-white/45 font-mono">
                                            {bureauShortCode(c.bureau)} · {c.type}
                                          </p>
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                          <span className="inline-flex items-center px-2 py-1 rounded-full border border-fuchsia-500/30 bg-fuchsia-500/10 text-fuchsia-200 text-[10px] font-black uppercase tracking-widest">
                                            {c.severity}
                                          </span>
                                          <span
                                            className={
              ...
              <div className="pt-4">
                <div className="fc-soft-surface-lg p-5 space-y-3">
                  <button
                    type="button"
                    className="w-full flex items-center justify-between gap-3 px-4 py-3 fc-light-chrome-btn transition-colors text-left"
                    ...
                    <div className="space-y-4">
                      {(() => {
                        ...
                        return entries.map(([type, list]) => {
                          ...
                          return (
                            <details key={type} className="group fc-soft-surface-lg p-4">
                              ...
                              <div className="mt-4 space-y-3">
                                <div className="flex flex-wrap items-center justify-between gap-2">
                                  <div className="text-[10px] uppercase tracking-widest text-white/45 font-mono">
                                    Export / capture
                                  </div>
                                  <div className="flex flex-wrap items-center gap-2">
                                    <button
      ...
      {tab === 'comparison' && (
        <div className="fc-soft-surface-lg backdrop-blur-xl p-6 space-y-4">
          <p className="text-[10px] uppercase tracking-widest text-white/45">Comparison mode</p>
            ...
            <div className="space-y-4">
              <div className="grid md:grid-cols-3 gap-4 items-end">
                <div className="md:col-span-2">
                  <label className="text-[10px] font-bold text-white/45 uppercase tracking-widest">Compare this report against</label>
                  <select
              ...
              {compareSummary ? (
                <div className="grid lg:grid-cols-2 gap-6">
                  ...
                  <div className="fc-soft-surface-lg p-5 space-y-3">
                    <div className="text-white/90 font-semibold">Score changes</div>
                    {compareSummary.scoreChanges.length ? (
                      ...
                      <div className="text-white/45 text-sm">No score rows available in either report.</div>
                    )}
                  </div>
                </div>
              ) : (
      ...
      {activeInsight && (
        <div className="fixed inset-0 z-[999] p-4 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setInsightsOpenId(null)} />
          <div
          ...
          >
            <div className="p-6 border-b border-white/[0.08] flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className={FINELY_OS_ENTITY_SUBLABEL}>Insights drawer</div>
                <div className={`mt-2 text-2xl font-light ${FINELY_OS_ENTITY_VALUE} truncate`}>{activeInsight.account}</div>
                <div className="mt-1 text-[10px] uppercase tracking-widest text-white/45 font-mono">
                  {bureauShortCode(activeInsight.bureau)} • {activeInsight.type} • severity {activeInsight.severity}
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 space-y-6 max-h-[72vh] overflow-y-auto">
              <div className={`${finelyOsCatalogCard('sky')} !p-4 fc-surface-harmony space-y-2`}>
                <div className="text-white/90 font-semibold">Why this is ranked high</div>
              ...
              <div className="fc-soft-surface-lg p-5 space-y-3">
                <div className="text-white/90 font-semibold">Key facts (detected)</div>
                {(() => {
              ...
              <div className="fc-soft-surface-lg p-5 space-y-3">
                <div className="text-white/90 font-semibold">Contradictions / dispute angles</div>
              ...
              <div className="fc-soft-surface-lg p-5 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="text-white/90 font-semibold">Evidence checklist</div>
                  <button
                    type="button"
                    onClick={openEvidence}
                    className="px-4 py-2 rounded-xl bg-amber-500 text-black font-black uppercase tracking-widest text-[10px] hover:brightness-110 transition-all"
                ...
                <div className="space-y-2">
                  {(activeInsight.insight?.evidenceChecklist ?? []).map((ev) => (
                    <div key={ev.id} className="fc-light-glass-panel fc-light-chrome-panel p-4">
                      <div className="text-white/90/85 font-semibold text-sm">{ev.title}</div>
