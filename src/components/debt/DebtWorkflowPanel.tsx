    ...
    <div className={`space-y-3 ${finelyOsGlassShell('panel', 'sky')}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className={`inline-flex items-center gap-2 ${FINELY_OS_ENTITY_SUBLABEL}`}>
          <Scale size={16} className="text-sky-300" /> FDCPA clocks
        </div>
        {hasUrgentClock && onOpenValidationDraft ? (
          <button
            type="button"
            className={`inline-flex items-center gap-2 ${FINELY_OS_SECONDARY_BTN} !py-2 !px-3 text-sm font-extrabold`}
        ...
        {timers.map((t) => (
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-sm font-bold opacity-90">
            ...
            <div className={`mt-1 text-base ${FINELY_OS_ENTITY_BODY}`}>
