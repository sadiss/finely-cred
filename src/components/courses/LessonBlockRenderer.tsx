...
function Block({ b }: { b: LessonContentBlock }) {
  ...
  if (b.type === 'image') {
    ...
    return (
      <div className="fc-light-glass-panel fc-light-chrome-panel p-4">
        {src ? (
          <img src={src} alt={txt(d.alt)} className="w-full rounded-xl border border-white/[0.08]" />
        ) : (
  ...
  if (b.type === 'faq') {
    ...
    return (
      <div className="space-y-2">
        {items.map((x, idx) => (
          <div key={idx} className="fc-light-glass-panel fc-light-chrome-panel p-5">
  ...
  if (b.type === 'accordion') {
    ...
    return (
      <div className="space-y-2">
        {items.map((x, idx) => (
          <details key={idx} className="fc-light-glass-panel fc-light-chrome-panel p-5">
...
function quizAnswerIndex(d: Record<string, unknown>): number | null {
  if (typeof d.answerIndex === 'number') return d.answerIndex;
  if (typeof d.correctIndex === 'number') return d.correctIndex;
  return null;
}

function quizExplain(d: Record<string, unknown>): string {
  return txt(d.explain ?? d.explanation ?? '');
}
...
function QuizMcq({ b }: { b: LessonContentBlock }) {
  ...
  const correct = quizAnswerIndex(d);
  const show = selected != null;
  const ok = show && correct != null && selected === correct;
  const explain = quizExplain(d);
  return (
    <div className="fc-light-glass-panel fc-light-chrome-panel p-5 space-y-3">
        ...
        {choices.map((c, idx) => (
          <button
            ...
            className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
              selected === idx ? 'border-amber-500/30 bg-amber-500/10 text-white' : 'border-white/[0.08] bg-white/[0.02] text-white/75 hover:bg-white/[0.05]'
            }`}
          >
            {c}
          </button>
      ...
      {show ? (
        <div className={`rounded-xl border p-3 text-sm ${ok ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-50' : 'border-rose-500/30 bg-rose-500/10 text-rose-50'}`}>
          ...
          {explain ? <div className="mt-2 text-white/80">{explain}</div> : null}
        </div>
      ) : null}
    </div>
  );
        ...
        {[
          ...
          <button
            ...
            className={`px-4 py-3 rounded-xl border text-sm transition-all ${
              selected === x.v ? 'border-amber-500/30 bg-amber-500/10 text-white' : 'border-white/[0.08] bg-white/[0.02] text-white/75 hover:bg-white/[0.05]'
            }`}
          >
            {x.label}
          </button>
