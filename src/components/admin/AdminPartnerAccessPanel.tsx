...
export function AdminPartnerAccessPanel({ partner, userRole, onUpdated }: Props) {
  ...
  const roleFields = useMemo(() => inviteFieldGuide(role === 'au_tradelines' ? 'au_seller' : role, partner.lane), [role, partner.lane]);
  ...
  const sendInvite = async () => {
    ...
    try {
      ...
      setNotice(`Invite email sent to ${email}. It opens account creation with their email and role context pre-filled.`);
    } catch (e: unknown) {
