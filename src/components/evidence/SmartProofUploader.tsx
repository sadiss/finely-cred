...
export function SmartProofUploader({
  ...
  return (
    <div className={finelyOsGlassShell('panel', 'emerald')} id="smart-proof-uploader">
      <div className={compact ? 'space-y-3' : 'border-b border-white/[0.08] pb-4 mb-4 space-y-2'}>
        ...
        {!compact ? (
          ...
          <p className={`${FINELY_OS_ENTITY_BODY} text-sm`}>
            Pick document type → camera or file → auto-classified to your evidence vault.
          </p>
        )}
      </div>
      ...
      <div className="mb-3">
        <label className={`block ${FINELY_OS_ENTITY_SUBLABEL} mb-1`}>Notes (optional — helps identification)</label>
        <input
          value={caption}
          onChange={(e) => setCaption(e.target.value)}
          placeholder={preset?.caption || 'e.g. Equifax Round 1 investigation results received today'}
          className={`${finelyOsGlowField('emerald')} w-full max-w-xl text-sm`}
        />
      </div>
      ...
      {result ? (
        <div className={`mt-4 ${finelyOsGlowPanel('emerald')} p-4 space-y-3`}>
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className={FINELY_OS_PRIMARY_BTN} onClick={() => navigate(result.profile.primaryRoute)}>
