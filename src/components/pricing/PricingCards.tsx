...
export function PackageCard({
  ...
  return (
    <div
      className="group relative rounded-2xl border p-6 flex flex-col h-full transition-all hover:border-amber-500/50 border-white/10 overflow-hidden"
      style={{
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {(pkg.scopeBullets?.length || pkg.scopeBulletsByRail?.stripe?.length || pkg.scopeBulletsByRail?.in_house?.length) ? (
          <details className={`mb-6 rounded-2xl border ${isPlatinum ? 'border-black/10 bg-white/40' : 'border-white/10 bg-black/25'} p-4`}>
...
export function AgencyTierCard({ tier, onSelect }: { tier: AgencyTier; onSelect: () => void }) {
  const isEnterprise = tier.monthlyPriceAmount === 0;
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        <div className="text-3xl font-bold text-white">{isEnterprise ? 'Custom' : formatPrice(tier.monthlyPriceAmount)}</div>
        {!isEnterprise && <div className="text-white/50 text-sm">/month</div>}
        {tier.annualPriceAmount && (
          <div className="text-emerald-400/80 text-xs mt-1">or {formatPrice(tier.annualPriceAmount)}/year (save 2 months)</div>
        )}
      </div>
      ...
      >
        {isEnterprise ? 'Contact us' : 'Get started'} <ArrowRight size={14} />
      </button>
    </div>
  );
}
