...
export function JourneyRoadmap(args: {
  ...
  const [view, setView] = useState<'map' | 'timeline' | 'cinematic'>(() => {
    ...
    return args.defaultView ?? 'timeline';
  });
  useEffect(() => {
    try {
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end gap-2">
        <button
          ...
          className={
            ...
            (view === 'map' ? 'bg-amber-500 text-black border-amber-400' : 'bg-white/5 text-white/70 border-white/10 hover:bg-white/10 hover:text-white')
          }
      ...
      {view === 'map' ? (
        <JourneyMapView stage={stage} signals={args.signals} lane={args.lane} />
      ) : view === 'cinematic' ? (
        <div className="space-y-3">
          {!cinematicAllowed ? (
            <div className="rounded-2xl border border-white/10 bg-black/30 backdrop-blur-xl p-5 text-white/60 text-sm">
        ...
        <div className="rounded-2xl border border-white/10 bg-black/30 backdrop-blur-xl p-6 overflow-hidden">
