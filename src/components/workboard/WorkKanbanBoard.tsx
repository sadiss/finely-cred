  ...
  function KanbanCard({ it, stageColor }: { it: WorkBoardItem; stageColor?: string }) {
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-3">
          ...
          {allowDrag ? (
            <button
              ...
              {...attributes}
              {...listeners}
              className="shrink-0 px-2 py-1 rounded-lg fc-light-glass-panel fc-light-chrome-panel border text-white/60 text-[10px] font-black uppercase tracking-widest hover:bg-white/[0.03]"
        ...
        {cardMeta ? (
          <div className="text-[11px] text-white/50">{cardMeta(it)}</div>
        ) : it.kind || it.priority || it.dueAt || it.workflowStageLabel ? (
          <WorkKanbanCardMeta item={it} />
        ) : null}
