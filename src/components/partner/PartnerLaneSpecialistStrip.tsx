...
export function PartnerLaneSpecialistStrip({ partnerId }: { partnerId: string }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-gradient-to-r from-violet-500/10 via-black/40 to-emerald-500/5 p-4 space-y-3">
      <div className="text-[10px] font-black uppercase tracking-widest text-white/45">Your specialists by lane</div>
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3">
        {cards.map((c) => (
          <button
            ...
            className="text-left rounded-xl border border-white/10 bg-black/30 p-3 hover:border-white/25 transition"
          >
            <div className="flex items-center gap-3">
              <StaffPortraitImg staff={c.staff} className="h-10 w-10 rounded-xl border border-white/15 shrink-0" />
              <div className="min-w-0">
                <div className="text-[10px] uppercase tracking-wider text-emerald-300/90 font-bold">{c.laneLabel}</div>
                <div className="text-sm font-bold text-white truncate">{staffMemberFullName(c.staff)}</div>
