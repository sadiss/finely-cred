...
export type FunnelLeadCaptureCopy = {
  accessLabel?: string;
  unlockTitle?: string;
    ...
    >
      {!embedded ? (
        <>
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.22em] text-amber-100/90">
                {copy?.accessLabel ?? 'Free access pass'}
              </p>
              <h3 className="mt-1 text-xl font-black leading-tight text-white">
                {copy?.unlockTitle ?? 'Unlock the guide and portal preview.'}
              </h3>
            </div>
          ...
          <div className="mb-4 grid grid-cols-3 gap-2">
            {(copy?.chips ?? ['PDF guide', 'Portal preview', '$0 today']).map((item) => (
      ...
      <form onSubmit={onSubmit} className={`space-y-2.5 ${FINELY_OS_MOBILE_FORM_COMPACT}`}>
        ...
        <label className="flex items-start gap-2 text-[11px] leading-snug text-white/45">
          ...
          {copy?.marketing ?? 'Send me credit tips by email (optional).'}
        </label>
        ...
        >
          {busy ? copy?.sending ?? 'Sending…' : submitLabel} <ArrowRight className="w-4 h-4" />
        </button>
        <div className="grid grid-cols-2 gap-2 text-[10px] text-white/45">
          <span className="inline-flex items-center justify-center gap-1 rounded-lg bg-white/[0.04] px-2 py-1.5">
            <Lock className="w-3 h-3" /> {copy?.noCard ?? 'No credit card'}
          </span>
          <span className="inline-flex items-center justify-center gap-1 rounded-lg bg-white/[0.04] px-2 py-1.5">
            <ShieldCheck className="w-3 h-3" /> {copy?.secure ?? 'Secure delivery'}
          </span>
        </div>
      </form>
