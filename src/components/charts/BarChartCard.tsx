...
export type BarSeries = { id: string; label: string; color?: string };

export function BarChartCard(props: {
  title: string;
  subtitle?: string;
  labels: string[];
  series: (BarSeries & { values: number[] })[];
  height?: number;
  stacked?: boolean;
}) {
      ...
      <div style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }} barCategoryGap="20%">
            <CartesianGrid stroke={CHART_PALETTE.grid} strokeDasharray={CHART_PALETTE.gridDash} vertical={false} />
            ...
            {props.series.map((s, i) => (
              <Bar
                ...
                dataKey={s.id}
                name={s.label}
                fill={s.color ?? defaultSeriesColors(i)}
                fillOpacity={0.85}
                radius={[6, 6, 0, 0]}
