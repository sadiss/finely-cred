...
export function CMOStaffConsole({ currentSignal, compact = false }: CMOStaffConsoleProps) {
  ...
  return (
    <section className="fc-panel relative overflow-hidden rounded-3xl border border-emerald-300/15 bg-slate-950/90 p-5 shadow-2xl shadow-black/30">
      ...
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-emerald-300/25 bg-emerald-300/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-emerald-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2 text-slate-300">
            <Sparkles className="mb-1 h-4 w-4 text-emerald-200" /> {settings.autonomyMode.replaceAll('_', ' ')}
          </div>
        </div>
      </div>
      ...
      <div className={compact ? 'max-h-80 space-y-3 overflow-auto pr-1' : 'max-h-[32rem] space-y-3 overflow-auto pr-1'}>
        {messages.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-emerald-300/25 bg-emerald-300/[0.04] p-5 text-sm leading-6 text-emerald-50/85">
            <MessageSquare className="mb-2 h-5 w-5 text-emerald-200" />
            ...
            <article
              ...
              className={
                message.role === 'owner'
                  ...
                  : 'max-w-4xl rounded-3xl border border-emerald-300/20 bg-white/[0.04] p-4 text-sm leading-6 text-slate-100'
              }
            >
              ...
              {message.actionItems.length > 0 && (
                <div className="mt-3 grid gap-2 md:grid-cols-2">
                  {message.actionItems.map((item) => (
                    <div key={item.id} className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.05] p-3">
      ...
      <div className="mt-4 flex flex-col gap-2 rounded-3xl border border-white/10 bg-black/25 p-2 sm:flex-row">
        <textarea
          ...
          className="min-h-20 flex-1 resize-none rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm text-white outline-none placeholder:text-slate-500 focus:border-emerald-300/50"
        />
