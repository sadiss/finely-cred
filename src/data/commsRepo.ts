...
export function hasRecentLeadCommsSend(args: {
  templateId: string;
  leadId: string;
  dedupeKey?: string;
  withinHours: number;
  ...
  const within = Math.max(0, Number(args.withinHours) || 0);
  if (!t || !leadId || !within) return false;
  const cutoff = Date.now() - within * 60 * 60 * 1000;
  ...
  return (
    store.sends.find((s) => {
      if (s.templateId !== t) return false;
      if (s.meta?.leadId !== leadId) return false;
      if (dedupeKey && s.meta?.dedupeKey !== dedupeKey) return false;
      if (s.status !== 'sent' && s.status !== 'dry_run') return false;
      const ts = Date.parse(s.createdAt);
      return Number.isFinite(ts) && ts >= cutoff;
    }) != null
  );
