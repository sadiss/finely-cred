...
export async function pullWorkflowSnapshotFromSupabase(args: { partnerId: string }) {
  ...
  if (reportsRes.error) return { ok: false as const, error: reportsRes.error.message };
  if (evidenceRes.error) return { ok: false as const, error: evidenceRes.error.message };
  if (lettersRes.error) return { ok: false as const, error: lettersRes.error.message };
  ...
  replaceEvidenceSnapshotForPartner({ partnerId, items: evidence });
  replaceLettersSnapshotForPartner({ partnerId, letters });
  replaceCasesSnapshotForPartner({ partnerId, cases });

  return { ok: true as const, reports: reports.length, evidence: evidence.length, letters: letters.length, cases: cases.length };
}
