...
export function logProspectWorkBridge(
  prospectId: string,
  meta: { partnerId: string; projectId?: string; packageId?: string },
): Prospect | null {
  const p = getProspect(prospectId);
  if (!p) return null;
  const touches = [
    touch('converted', { partnerId: meta.partnerId, packageId: meta.packageId }),
    ...(meta.projectId ? [touch('work_linked', { projectId: meta.projectId, partnerId: meta.partnerId })] : []),
    ...(p.touches ?? []),
  ];
  return upsertProspect({ ...p, touches });
}
