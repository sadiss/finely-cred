...
export function grantEntitlementsFromPackage(args: {
  ...
  const derivedModuleKeys = (() => {
    ...
    if (pkg.category === 'tradeline_promo') {
      return [...base, 'portal.disputes', 'portal.letters', 'portal.templates', 'tradeline_starter'];
    }
  ...
  try {
    provisionWorkFromPurchase({ partnerId: args.partnerId, packageId: args.packageId });
  } catch {
    // non-blocking — entitlements still granted
  }
...
/**
 * Always-free (promo) modules. Uploading a credit report and generating/viewing a
 * credit analysis report are free for everyone — no plan or trial required.
 */
const FREE_ENTITLEMENT_KEYS = new Set<string>(['portal.reports', 'portal.documents']);

/**
 * Check if a partner has a specific entitlement (active)
 */
export function hasEntitlement(partnerId: string, key: string): boolean {
  if (FREE_ENTITLEMENT_KEYS.has(key)) return true;
