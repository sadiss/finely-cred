...
function buildSections(args: {
  ...
  const scoreLine = scores.length
    ? scores.map((s) => `${s.model}${s.bureau ? ` (${s.bureau})` : ''}: ${s.value}`).join(' • ')
  ...
  const sections: Section[] = [
    {
      ...
      bullets: [
        ...
        `Potential dispute candidates: ${neg.length}`,
      ],
    },
    {
      title: 'Now (0–7 days)',
  ...
  for (const k of keys) {
    ...
    sections.push({
      title: `Negatives — ${k}`,
      bullets: items.slice(0, max).map((x) => `${x.account} • ${x.status} • code ${x.code}`),
    });
  }
