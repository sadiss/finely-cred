...
const MODULE_CARDS = [
  { path: '/business/profile', title: 'Business profile', body: 'Entity, EIN, NAICS, address hygiene — the identity layer funders verify first.', cta: 'Open profile', icon: Building2, accent: 'violet' as const },
  ...
  { path: '/business/disputes', title: 'Business disputes', body: 'Challenge inaccurate commercial bureau entries with letter workflow.', cta: 'Dispute center', icon: ShieldCheck, accent: 'rose' as const },
];
...
export default function BusinessDashboardPage() {
  ...
  const kpis = useMemo(() => {
    if (!partner) {
      return [
        { label: 'Foundation', value: '—', hint: 'Entity + EIN', accent: 'emerald' as const },
    ...
    return [
      { label: 'Foundation', value: `${foundation.percent}%`, hint: foundation.complete ? 'Entity signals aligned' : 'Profile hygiene', accent: 'emerald' as const },
          ...
          <FinelyUnifiedSection title="How fundability is built" subtitle="Sequencing beats random applications.">
            ...
            <div className="mt-4 grid md:grid-cols-3 gap-3">
              {[
                ...
                <div key={t} className={finelyOsCatalogCard((['emerald', 'violet', 'sky'] as const)[idx % 3])} data-fc-accent={(['emerald', 'violet', 'sky'] as const)[idx % 3]}>
                  <Icon size={22} className="text-violet-700 mb-3" />
                  <div className={`font-extrabold text-xl ${FINELY_OS_ENTITY_VALUE}`}>{t}</div>
                  <p className={`mt-2 text-base font-semibold ${FINELY_OS_ENTITY_BODY}`}>{d}</p>
                </div>
              ))}
            </div>
          </FinelyUnifiedSection>
          ...
          <FinelyUnifiedSection title="Business credit modules" subtitle="Jump to any workstation.">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {MODULE_CARDS.map((item) => {
                ...
                return (
                  <button key={item.path} type="button" onClick={() => navigate(item.path)} className={`text-left ${finelyOsCatalogCard(item.accent)}`} data-fc-accent={item.accent}>
                    <Icon size={16} className={`mb-2 ${item.accent === 'violet' ? 'text-violet-700' : item.accent === 'rose' ? 'text-rose-700' : item.accent === 'emerald' ? 'text-emerald-700' : item.accent === 'sky' ? 'text-sky-700' : 'text-fuchsia-700'}`} />
