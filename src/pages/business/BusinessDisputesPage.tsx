...
import {
  businessDisputeCaseHref,
  isBusinessDisputeProductPreviewPath,
} from '../../lib/businessDisputeProductPaths';
import {
...
export function BusinessDisputesWorkspace({ embedded = false }: { embedded?: boolean }) {
  const navigate = useNavigate();
  const { pathname, search } = useLocation();
  const previewDisputes = embedded || isBusinessDisputeProductPreviewPath(pathname);
  ...
  const openDispute = (disputeId: string) => navigate(businessDisputeCaseHref(disputeId, pathname, search));

  const workspaceBody = (
    <div className={FINELY_OS_PAGE}>
      {!previewDisputes ? <BusinessNav /> : null}
      {!previewDisputes ? <BusinessCommandStrip partner={partner ?? null} /> : null}
      ...
      <FinelyUnifiedHubLayout
        ...
        accent="rose"
        kpis={[
          { label: 'Disputes', value: String(disputes.length), accent: 'rose' },
      ...
      >
        {tab === 'queue' && !partner && (
          <div className={FINELY_OS_NOTICE}>Sign in as a partner to create and track business disputes.</div>
        )}
              ...
              {disputes.length === 0 ? (
                ...
                <FinelyOsPaginatedStack
                  ...
                  renderItem={(d, idx) => (
                    <div
                      key={d.id}
                      className={finelyOsCatalogCard((['emerald', 'violet', 'sky', 'rose'] as const)[idx % 4])}
                      data-fc-accent={(['emerald', 'violet', 'sky', 'rose'] as const)[idx % 4]}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <button type="button" onClick={() => openDispute(d.id)} className="min-w-0 text-left">
        ...
        {tab === 'create' && (
          <>
            {!partner ? (
              ...
              <div className={`${finelyOsCatalogCard('violet')} space-y-4 max-w-xl`} data-fc-accent="violet">
  ...
  if (embedded) {
    return (
      <div className="fc-wlp-business-dispute-workspace-embed" data-surface-kind="real" data-surface-key="partner:business-disputes">
        {workspaceBody}
      </div>
    );
  }
...
export default function BusinessDisputesPage() {
  return <BusinessDisputesWorkspace />;
}
