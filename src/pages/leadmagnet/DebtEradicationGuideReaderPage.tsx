...
export default function DebtEradicationGuideReaderPage() {
  ...
  return (
    <GuideReaderShell
      className="dge-shell relative overflow-x-hidden selection:bg-[#7c2027]/35"
      ...
      storageKey="finely.guideReader.debtEradication"
      maxWidthClassName="max-w-[86rem]"
      headerClassName="dge-dock"
      progressTrackClassName="dge-dock-rule"
      headerLeft={
        <>
          <button
            type="button"
      ...
      renderChapter={(i) => {
        const ch = DEBT_GUIDE_CHAPTERS[i] ?? DEBT_GUIDE_CHAPTERS[0]!;
        return (
          <article className="dge-leaf dge-turn">
            <div className="dge-leaf-inner">
              <div className="dge-caption">
                <div className="dge-caption-row">
                  ...
                  <span>{ch.readMinutes} min read</span>
                </div>
                <div className="mt-4 flex items-start gap-5">
                  ...
                  <div className="min-w-0">
                    <span className="dge-chapter-numeral sm:hidden">{ch.number}</span>
                    <h1 className="dge-chapter-title">{ch.title}</h1>
                    <div className="dge-rule-draw mt-3 w-32" aria-hidden />
                    <p className="dge-chapter-sub mt-3 max-w-2xl">{ch.subtitle}</p>
                  </div>
                </div>
                <div className="dge-promise mt-5">{ch.promise}</div>
              </div>

              <div className="mt-7">
                {ch.sections.map((section, si) => (
                  <GuideSectionView key={section.heading} section={section} prefix="dge" index={si} />
                ))}
              </div>

              <div className="dge-takeaway">
                <div className="dge-takeaway-label">Page {ch.number} — hold this</div>
                <p className="dge-takeaway-body">{ch.takeaway}</p>
              </div>
              ...
              <div className="dge-foot-nav flex flex-wrap items-center justify-between gap-3">
                ...
                {i < TOTAL - 1 ? (
                  ...
                  >
                    {DEBT_GUIDE_CHAPTERS[i + 1]!.number}. Next page <ChevronRight size={16} />
                  </button>
                ) : (
                  <Link
                    to={`${DEBT_GUIDE_LANDING_PATH}#download`}
