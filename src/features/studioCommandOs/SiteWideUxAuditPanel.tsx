  ...
  return (
    <div className="space-y-6">
      ...
      <StudioSection eyebrow="site-wide UX command rules" title="Kill cramped side-by-side layouts and endless list UIs across the admin system.">
        ...
        <div className="grid lg:grid-cols-2 gap-4">{remedies.map((r) => <div key={r.id} className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 space-y-4"><div className="flex items-start justify-between gap-3"><div><div className="text-[10px] uppercase tracking-widest text-sky-300 font-black">{r.priority}</div><div className="mt-2 text-white font-black text-lg">{r.title}</div></div><LayoutDashboard className="text-white/35" size={22} /></div><div className="grid md:grid-cols-2 gap-3"><div className="rounded-2xl border border-rose-400/15 bg-rose-500/10 p-4"><div className="text-[10px] uppercase tracking-widest text-rose-200/80">Before</div><div className="mt-2 text-sm text-white/60 leading-relaxed">{r.before}</div></div><div className="rounded-2xl border border-emerald-400/15 bg-emerald-500/10 p-4"><div className="text-[10px] uppercase tracking-widest text-emerald-200/80">After</div><div className="mt-2 text-sm text-white/60 leading-relaxed">{r.after}</div></div></div><div className="rounded-2xl border border-white/10 bg-black/30 p-4 text-sm text-white/65"><strong className="text-white">Cursor action:</strong> {r.action}</div></div>)}</div>
      </StudioSection>
      <div className="grid lg:grid-cols-2 gap-6">
        <StudioSection eyebrow="rules" title="Long-list replacement system" right={<ListChecks className="text-violet-300" />}>
          <div className="space-y-3">{LONG_LIST_REPLACEMENT_RULES.map((r, idx) => <div key={r} className="rounded-2xl border border-white/10 bg-black/30 p-4 flex gap-3"><div className="shrink-0 h-7 w-7 rounded-full bg-violet-500 text-white font-black grid place-items-center text-xs">{idx + 1}</div><div className="text-white/70 text-sm leading-relaxed">{r}</div></div>)}</div>
        </StudioSection>
        <StudioSection eyebrow="primary actions" title="Every page gets one obvious next move" right={<Sparkles className="text-violet-300" />}>
