...
export const AUTOMATION_BLUEPRINTS: AutomationBlueprint[] = [
  ...
  {
    id: 'bp-letter-mailed-followup',
    title: 'Letter Mailed → Bureau Timer',
    category: 'nurture',
    summary: 'When a dispute letter is mailed, starts bureau response timer, partner success milestone, and follow-up task.',
    bestFor: ['Round 1–3 dispute letters', 'certified mail', 'partner vault'],
    owner: 'Dispute Processing Specialist + Inbox Triage',
    expectedOutcome: 'Partners know when to expect bureau response and upload replies.',
    nodes: [
      node('trigger', 'trigger', 'Letter mailed', 'Partner or admin mail action', 'Emit letter.mailed platform event.'),
      node('milestone', 'action', 'Success milestone', 'Letter prep review module', 'Unlock partner success review prompt.'),
      node('task', 'action', 'Follow-up task', '35-day bureau window', 'Create pending follow-up with upload reminder.'),
      node('email', 'action', 'Portal nudge', 'If email consent', 'Send educational timing expectations — no outcome promises.'),
      node('exit', 'exit', 'Case timeline', 'Sync dispute case round', 'Mark round mailed in case workspace.'),
    ],
    recommendedCaps: ['No legal advice in auto copy', 'One follow-up email max', 'Respect partner timezone'],
    complianceNotes: baseCompliance,
  },
  {
    id: 'bp-bankruptcy-home-retention',
    title: 'Bankruptcy Home Retention Nurture',
    category: 'nurture',
    summary: 'Partners exploring bankruptcy home retention get scenario guidance, coach routing, and document checklist.',
    bestFor: ['foreclosure timeline', 'Chapter 13 cure', 'sale date approaching'],
    owner: 'Bankruptcy Specialist + Ruth Co-Owner oversight',
    expectedOutcome: 'Partner understands liberation paths without guaranteed outcomes.',
    nodes: [
      node('trigger', 'trigger', 'Bankruptcy lane opened', 'Portal bankruptcy hub', 'Detect scenario selection or coach chat start.'),
      node('guide', 'action', 'Liberation path email', 'Educational only', 'Send scenario summary + document checklist link.'),
      node('coach', 'action', 'On-duty specialist', 'Lane hash routing', 'Assign bankruptcy lane specialist — not duplicate coaches.'),
      node('review', 'approval', 'Human review', 'High-stakes copy', 'Velvet Hammer gate on any funding/home-save claims.', 'high'),
      node('exit', 'exit', 'Portal milestone', 'Liberation path chosen', 'Record partner success milestone.'),
    ],
    recommendedCaps: ['Never promise home saved', 'Not legal advice disclaimer', 'Escalate litigation to counsel'],
    complianceNotes: baseCompliance,
  },
  {
    id: 'bp-comms-sequence-enroll',
    title: 'Comms Sequence Auto-Enroll',
    category: 'nurture',
    summary: 'Enrolls new leads into the right nurture sequence based on funnel, city, and consent flags.',
    bestFor: ['lead magnet capture', 'CRM stage new', 'Meta lead forms'],
    owner: 'Liora Lifecycle + Switchboard',
    expectedOutcome: '300+ template depth used automatically — not a thin drip.',
    nodes: [
      node('trigger', 'trigger', 'Lead captured', 'Funnel or CRM', 'Read source, city, lane, consent.'),
      node('match', 'condition', 'Sequence matcher', 'Nurture catalog', 'Pick sequence from commsSequencesRepo by lane.'),
      node('enroll', 'action', 'Enroll sequence', 'Dry-run first', 'Queue step 1 with merge fields and compliance footer.'),
      node('guard', 'condition', 'TCPA/CAN-SPAM', 'Consent required', 'Block SMS without proof; email needs unsubscribe.'),
      node('exit', 'exit', 'Comms timeline', 'Log to inbox', 'Surface in Comms Studio send log.'),
    ],
    recommendedCaps: ['Dry-run default', 'Max 1 SMS / 48h', 'Full template library — no 60-cap'],
    complianceNotes: baseCompliance,
  },
];
