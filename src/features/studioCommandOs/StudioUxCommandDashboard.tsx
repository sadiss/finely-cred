...
export function StudioUxCommandDashboard() {
  ...
  return <div className="space-y-6">
    <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-violet-500/12 via-white/[0.04] to-black/40 p-5 md:p-7">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div><div className="inline-flex items-center gap-2 text-violet-300 text-xs uppercase tracking-[0.24em] font-black"><Bot size={16} /> Studio UX Command OS</div><h1 className="mt-3 text-3xl md:text-5xl font-black text-white tracking-tight">Make the admin feel expensive, obvious, and powerful.</h1><p className="mt-4 max-w-4xl text-white/60 leading-relaxed">This layer removes cramped two-column studio layouts, replaces ridiculous long lists with command decks, upgrades the video generator to a prompt-first workflow, and gives Automation Studio scenario-based blueprints.</p></div>
      </div>
      <div className="mt-6 grid sm:grid-cols-2 xl:grid-cols-6 gap-3">{tabs.map((t) => <button key={t.id} type="button" onClick={() => setTab(t.id)} className={`rounded-3xl border p-4 text-left transition-all ${tab === t.id ? 'border-violet-400/40 bg-violet-500/10' : 'border-white/10 bg-black/30 hover:bg-white/[0.04]'}`}><div className="inline-flex items-center gap-2 text-white font-bold">{t.icon}{t.label}</div><div className="mt-2 text-xs text-white/45 leading-relaxed">{t.desc}</div></button>)}</div>
    </div>
