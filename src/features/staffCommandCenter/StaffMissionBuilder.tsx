  ...
  return (
    <div className={STAFF_CMD_PANEL}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className={`inline-flex items-center gap-2 ${STAFF_CMD_EYEBROW}`}>
            <ClipboardList size={16} /> Mission Builder
          </div>
          <h2 className={`mt-2 ${STAFF_CMD_TITLE}`}>Tell the staff what to do</h2>
          <p className={`mt-2 max-w-3xl text-sm ${STAFF_CMD_BODY}`}>
            Pick the mission, cities, intensity, and 1–3 staff. The system explains who owns it, what happens first, and what is blocked until approval.
          </p>
        </div>
        <button type="button" onClick={createMission} className={STAFF_CMD_PRIMARY_BTN}>
          <Play size={14} /> Create mission
        </button>
      </div>

      {notice ? (
        <div className="rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm text-emerald-100">{notice}</div>
      ) : null}
      ...
      <div className="space-y-6">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {missionOptions.map((m) => (
              <button
                ...
                className={`text-left rounded-xl border p-3 transition-all ${staffCmdSelected(missionType === m.id)}`}
              >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <select
              value={intensity}
              onChange={(e) => setIntensity(e.target.value as StaffMissionIntensity)}
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <select
              value={riskLevel}
              onChange={(e) => setRiskLevel(e.target.value as StaffRiskLevel)}
              className="mt-2 w-full rounded-xl border border-white/10 bg-black/30 px-3 py-3 text-sm text-white/80"
            >
        ...
        <div className={`${staffCmdRecommendPanel()} p-5`}>
          <div className="text-[10px] uppercase tracking-widest text-emerald-200 font-black">Recommended staff</div>
          <div className="mt-4 space-y-3">
            {recommended.map((s, idx) => (
              <div key={s.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/25 p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-white/50">{s.title}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="mt-2 text-white font-bold text-lg">{preview.request.title}</div>
          <p className={`mt-2 text-sm ${STAFF_CMD_BODY}`}>{preview.commandSummary}</p>
          {preview.request.approvalRequired ? (
            <div className="mt-3 flex items-start gap-2 rounded-xl border border-sky-500/25 bg-sky-500/10 p-3 text-xs text-sky-100">
              <ShieldAlert size={15} /> Approval required before risky/external actions.
            </div>
          ) : null}
          <div className="mt-4 space-y-2">
            {preview.firstThreeSteps.map((s) => (
              <div key={s} className="rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-white/65">
                {s}
              </div>
            ))}
          </div>
          <button type="button" onClick={createMission} className={`mt-4 w-full ${STAFF_CMD_PRIMARY_BTN}`}>
            {preview.suggestedNextButton} <ArrowRight size={14} />
          </button>
        </div>
      </div>
      ...
      <textarea
        ...
        placeholder="Optional: tell the staff exactly what you want this mission to accomplish…"
        className="min-h-[96px] w-full rounded-2xl border border-white/10 bg-black/30 p-4 text-sm text-white/80 outline-none placeholder:text-white/30"
      />
    </div>
  );
}
