...
import { StaffSocialPageAssignWizard } from '../../../../staffCommandCenter/StaffSocialPageAssignWizard';
import { buildMetaOAuthRedirectUris, primaryMetaOAuthRedirectUri } from '../../../../../lib/metaOAuthUrls';
import { SocialDisclosureReviewPanel } from '../../../../social/SocialDisclosureReviewPanel';
import { processRecruitingAutopilotTick } from '../../../../../lib/recruitingSopAutopilot';
import { FinelyOsPaginatedStack } from '../../../../os/FinelyOsPaginatedStack';
import { SocialWorkflowWeekStrip } from '../../../../social/SocialWorkflowWeekStrip';
import {
...
export type SocialHubEmbedTab = 'dashboard' | 'composer' | 'calendar' | 'inbox' | 'autopilot' | 'sop' | 'settings';

type SocialHubEmbeddedPanelProps = {
  activeTab?: SocialHubEmbedTab;
  onTabChange?: (tab: SocialHubEmbedTab) => void;
};

const TAB_LABELS: Record<SocialHubEmbedTab, string> = {
  dashboard: 'Overview',
  composer: 'Composer',
  calendar: 'Calendar',
  inbox: 'Inbox',
...
function parseTab(raw: string | null): SocialHubEmbedTab {
  if (raw === 'composer' || raw === 'calendar' || raw === 'inbox' || raw === 'settings' || raw === 'autopilot' || raw === 'sop') {
    return raw;
  }
  return 'composer';
}

/** Social Hub scheduling tools without PageShell — for growth workstation compose studio. */
export function SocialHubEmbeddedPanel({ activeTab, onTabChange }: SocialHubEmbeddedPanelProps) {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [internalTab, setInternalTab] = useState<SocialHubEmbedTab>(() => parseTab(searchParams.get('tab')));
  const tab = activeTab ?? internalTab;

  const setTab = (next: SocialHubEmbedTab) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setNotice(`Meta connected — ${data.pages?.length ?? 0} page(s) synced.`);
        setTab('settings');
      } catch (e: unknown) {
  ...
  const stats = useMemo(() => {
    ...
    const reviewQueue = posts.filter((p) => p.status === 'needs_review' || p.complianceStatus === 'needs_review');
    const dueToPublish = posts.filter((p) => p.status === 'queued' && Date.parse(p.scheduledAt) <= Date.now());
    return {
  ...
  const handleSimulateLead = () => {
    ...
    refreshLocal();
    setNotice('Simulated Meta lead added to inbox.');
    setTab('inbox');
  };
  ...
  return (
    <div className="fc-wlp-growth-social-embed">
      {notice ? (
        <div className="fc-wlp-growth-social-notice" data-fcm-accent="emerald">
          {notice}
        </div>
      ) : null}
      ...
      {!isMetaIntegrationLive() ? (
        <div className={`${FINELY_OS_NOTICE_WARN} flex flex-wrap items-center justify-between gap-3`}>
          <p className="text-sm">Meta is in demo mode — connect OAuth in Settings or simulate leads for dev.</p>
          <button type="button" onClick={() => setTab('settings')} className="fc-wlp-btn-secondary">
            Meta settings
          </button>
        </div>
      ) : null}
      ...
      {tab === 'dashboard' && (
        <div className="fc-wlp-growth-social-kpi-grid">
          {[
            { label: 'Scheduled today', value: stats.scheduledToday, accent: 'emerald' as const },
            { label: 'Inbox unread', value: stats.inboxUnread, accent: 'violet' as const },
            { label: 'Leads this week', value: stats.leadsThisWeek, accent: 'sky' as const },
            { label: 'Connected pages', value: stats.connectedPages, accent: 'rose' as const },
          ].map((s) => (
            <div key={s.label} className="fc-wlp-growth-command-kpi" data-fcm-accent={s.accent}>
      ...
      {tab === 'composer' && (
        <div className="fc-wlp-growth-social-composer" data-fcm-accent="violet">
          <p className="fc-wlp-growth-rail-label">New post</p>
          <textarea
            value={caption}
          ...
          <div className="grid md:grid-cols-2 gap-3 mt-3">
            ...
            <div className="flex items-end gap-2 flex-wrap">
              <button type="button" onClick={() => navigate('/admin/content-studio')} className="fc-wlp-btn-secondary">
                Media Studio
              </button>
              <button type="button" className="fc-wlp-btn-primary" onClick={handleQueuePost}>
                <Plus size={14} aria-hidden /> Queue post
              </button>
            </div>
          </div>
        </div>
      ...
      {tab === 'calendar' && (
        <div className="fc-wlp-growth-social-calendar" data-fcm-accent="emerald">
          <p className="fc-wlp-growth-rail-label">Content calendar</p>
          <p className={`${FINELY_OS_ENTITY_BODY} mt-2`}>
            {posts.length === 0
              ? 'No queued posts yet — use Composer or connect Meta.'
      ...
      {tab === 'autopilot' && (
        <div className="space-y-4">
          ...
          <div className="fc-wlp-growth-social-autopilot" data-fcm-accent="violet">
            <p className="fc-wlp-growth-rail-label">Social autopilot</p>
            <p className={`${FINELY_OS_ENTITY_BODY} mt-2`}>
              Brand roles draft SOP-based captions on platform cron. Enable live publish after Meta connect.
            </p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                ...
                onClick={() => {
                  ...
                  refreshLocal();
                  setNotice(
                    `Autopilot: ${result.generated} queued · ${result.skipped} skipped · ${result.complianceBlocked} blocked · ${result.published} published`,
                  );
                }}
              >
                <Bot size={14} aria-hidden /> Run autopilot now
              </button>
              <button
                type="button"
                ...
                onClick={() => {
                  void (async () => {
                    ...
                    refreshLocal();
                    setNotice(`Published ${result.published} due post(s) locally`);
                  })();
                }}
              >
                <Megaphone size={14} aria-hidden /> Publish due now
              </button>
              <button
                type="button"
                className="fc-wlp-btn-secondary"
              ...
              <button type="button" className="fc-wlp-btn-secondary" onClick={() => setTab('sop')}>
                SOP library
              </button>
            </div>
      ...
      {tab === 'sop' && (
        <div className="fc-wlp-growth-social-sop-grid">
          <div className="fc-wlp-growth-social-sop-list" data-fcm-accent="violet">
            <p className="fc-wlp-growth-rail-label">Content SOP templates</p>
            <FinelyOsPaginatedStack
          ...
          <div className="fc-wlp-growth-social-sop-preview" data-fcm-accent="emerald">
            {(() => {
              ...
              return (
                <>
                  <p className="fc-wlp-growth-rail-label">Preview & queue</p>
                  {staff ? (
                    <div className="mt-2 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className={`${FINELY_OS_ENTITY_BODY} text-xs`}>Assigned voice</div>
                      </div>
                    </div>
                  ) : null}
                  <pre className={`mt-4 whitespace-pre-wrap text-sm ${FINELY_OS_ENTITY_BODY} ${finelyOsCatalogCard('sky')} max-h-64 overflow-y-auto`}>{draft}</pre>
                  {!review.ok ? (
                  ...
                  <button
                    type="button"
                    className="fc-wlp-btn-primary mt-4"
                    onClick={() => {
                  ...
                  >
                    <Plus size={14} aria-hidden /> Queue this SOP post
                  </button>
                </>
              );
            })()}
      ...
      {tab === 'inbox' && (
        <div className="fc-wlp-growth-social-inbox" data-fcm-accent="rose">
          <p className="fc-wlp-growth-rail-label">Unified inbox</p>
          <p className={`${FINELY_OS_ENTITY_BODY} mt-2`}>Meta DMs and lead forms — production messages land via meta-webhook.</p>
          <button type="button" onClick={() => navigate('/admin/support?source=meta')} className="fc-wlp-btn-secondary mt-3">
            Open partner conversations
          </button>
          {inbox.length === 0 ? (
            <p className="mt-4 text-sm text-white/45">No messages yet. Simulate a lead in Settings or connect Meta webhooks.</p>
          ) : (
            <ul className="mt-4 space-y-2">
              {inbox.map((m) => (
                <li key={m.id} className={`${finelyOsInlineListItem()} px-3 py-2 text-sm`}>
                  ...
                  <div className={`mt-1 ${FINELY_OS_ENTITY_BODY}`}>{m.text}</div>
                </li>
              ))}
            </ul>
          )}
      ...
      {tab === 'settings' && (
        <div className="space-y-4">
          ...
          <div className="fc-wlp-growth-social-settings" data-fcm-accent="violet">
            <p className="fc-wlp-growth-rail-label">Meta connections</p>
            <p className={`${FINELY_OS_ENTITY_BODY} mt-1`}>Status: {config.status}</p>
            {config.connectedPages.length > 0 ? (
              <ul className={`mt-2 text-sm space-y-1 ${FINELY_OS_ENTITY_BODY}`}>
            ...
            <div className="mt-4 space-y-3 max-w-lg">
              ...
              <div className="rounded-xl border border-violet-500/20 bg-violet-500/10 p-3 space-y-2">
                <div className={`text-xs font-semibold ${FINELY_OS_ENTITY_SUBLABEL} text-violet-200`}>OAuth redirect URIs</div>
                ...
                {buildMetaOAuthRedirectUris(config).map((uri) => (
                  <code key={uri} className="block text-[10px] break-all text-white/50">
                    {uri}
                  </code>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={connectMeta} className="fc-wlp-btn-primary">
                  Connect Meta account
                </button>
                <button type="button" onClick={saveSettings} className="fc-wlp-btn-secondary">
                  Save settings
                </button>
...
export function SocialHubToolRail({
  activeTab,
  onTabChange,
}: {
  activeTab: SocialHubEmbedTab;
  ...
  const tabs: Array<{ id: SocialHubEmbedTab; icon: typeof Sparkles }> = [
    { id: 'composer', icon: Megaphone },
    { id: 'calendar', icon: Calendar },
    { id: 'inbox', icon: Inbox },
    { id: 'autopilot', icon: Zap },
    { id: 'sop', icon: ClipboardList },
    { id: 'settings', icon: Settings },
  ];

  return (
    <nav className="fc-wlp-growth-social-tool-rail" aria-label="Social tools">
      {tabs.map((item, index) => {
