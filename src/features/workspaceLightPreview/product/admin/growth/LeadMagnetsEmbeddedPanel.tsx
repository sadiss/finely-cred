...
/** Lead magnet funnel editor — catalog, copy, hero media, urgency. */
export function LeadMagnetsEmbeddedPanel() {
  ...
  const override = useMemo(() => getFunnelOverride(base.funnelId) ?? getFunnelOverride(base.id), [selectedId, version, base.funnelId, base.id]);
  const media = useMemo(() => getFunnelMedia(base.funnelId) ?? getFunnelMedia(base.id), [selectedId, version, base.funnelId, base.id]);
  ...
  return (
    <div className="fc-wlp-growth-magnets-embed space-y-4">
      {notice ? (
        <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-4 py-3 text-sm font-bold text-emerald-100">{notice}</div>
      ) : null}
      {videoErr ? (
        <div className="rounded-xl border border-rose-500/25 bg-rose-500/10 px-4 py-3 text-sm font-bold text-rose-100">{videoErr}</div>
      ) : null}

      <div className="fc-wlp-growth-magnets-catalog" data-fcm-accent="emerald">
        <p className="fc-wlp-growth-rail-label">Funnel catalog</p>
        <p className={`${FINELY_OS_ENTITY_BODY} text-base mb-4`}>
          {LEAD_MAGNET_FUNNELS.length} funnels · {overrideCount} with admin overrides
        </p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2">
          {LEAD_MAGNET_FUNNELS.map((f) => (
            ...
            >
              <div className={`${FINELY_OS_ENTITY_BODY} font-bold text-sm`}>{f.metaTitle}</div>
      ...
      <FinelyOsGlassPanel icon={FileText} title={`Edit · ${base.metaTitle}`} accent="violet">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <textarea value={metaDesc} onChange={(e) => setMetaDesc(e.target.value)} rows={4} className={`${FINELY_OS_ENTITY_INPUT} w-full min-h-[120px]`} />
          </div>
        </div>
      ...
      <FinelyOsGlassPanel icon={Film} title="Hero media · video + imagery" accent="sky">
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          <div className="rounded-2xl border border-white/10 overflow-hidden bg-black/30 min-h-[240px]">
            <LeadMagnetFunnelHeroVideo config={base} theme={theme} posterUrl={heroPreview} className="!rounded-none" />
          </div>
          <div className={`space-y-3 text-base ${FINELY_OS_ENTITY_BODY}`}>
      ...
      <FinelyOsGlassPanel icon={Sparkles} title="Urgency controls" accent="rose">
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <label className={`flex items-end gap-2 pb-2 ${FINELY_OS_ENTITY_BODY} text-base font-bold`}>
