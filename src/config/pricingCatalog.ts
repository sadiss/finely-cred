...
export interface AgencyTier {
  ...
  /** Max team seats */
  seatLimit: number;
  /** Revenue-share pricing model — no separate platform access fee */
  pricingModel?: 'revenue_share' | 'custom';
  /** Phase-specific split examples — clearer than a single min–max range */
  splitBreakdown?: AgencyTierSplitRow[];
  /** Derived bounds: platform % at certified partner (low) and during training (high) */
  platformShareMinPct?: number;
  platformShareMaxPct?: number;
  /** White-label maturity tied to this capacity tier */
  whiteLabelLevel?: 'finely_branded' | 'co_branded' | 'white_label' | 'enterprise_white_label';
  /** Default training phase when joining on this tier */
  recommendedTrainingPhase?: 'apprenticeship' | 'guided' | 'independent' | 'partner';
  features: string[];
  isPublic: boolean;
  sortOrder: number;
  badge?: string;
...
export const bundlePackages: PricingPackage[] = [
  ...
  },
  {
    ...
    highlights: [
      ...
      '1× 30‑minute strategy call (sequencing + next steps)',
      'Priority support',
    ],
    highlightsByRail: {
      stripe: [
  ...
  },
  {
    ...
    highlights: [
      ...
      '1× 60‑minute free strategy call (routing + underwriting-ready next actions)',
      'Dedicated case manager',
    ],
...
// 2. Include bonus value (ebook, strategy call)
...
export const tradelinePromoPackages: PricingPackage[] = [
  ...
  },
  {
    ...
    description:
      'Three authorized user tradelines, primary installment tradeline reporting, plus a 30-minute strategy call.',
    highlights: [
...
export const agencyTiers: AgencyTier[] = [
  {
    ...
    whiteLabelLevel: 'finely_branded',
  ...
  },
  {
    ...
    pricingModel: 'revenue_share',
    splitBreakdown: [
      { label: 'While training', description: 'Shared fulfillment & QA checkpoints', agentKeepPct: 42, platformPct: 58 },
      { label: 'When certified', description: 'You run sales; co-branded portal live', agentKeepPct: 52, platformPct: 48 },
    ],
    platformShareMinPct: 48,
    platformShareMaxPct: 58,
  ...
  },
  {
    ...
    pricingModel: 'revenue_share',
    splitBreakdown: [
      { label: 'While training', description: 'Team workflows with mentor QA', agentKeepPct: 46, platformPct: 54 },
      { label: 'When certified', description: 'You operate volume; Finely on platform levers', agentKeepPct: 58, platformPct: 42 },
    ],
    platformShareMinPct: 42,
    platformShareMaxPct: 54,
    whiteLabelLevel: 'co_branded',
    recommendedTrainingPhase: 'guided',
    features: [
  ...
  },
  {
    ...
    pricingModel: 'revenue_share',
    splitBreakdown: [
      { label: 'Ramp-up', description: 'Automation + quarterly strategy support', agentKeepPct: 50, platformPct: 50 },
      { label: 'Certified partner', description: 'High-volume white-label at scale', agentKeepPct: 58, platformPct: 42 },
    ],
    platformShareMinPct: 42,
    platformShareMaxPct: 50,
    whiteLabelLevel: 'white_label',
  ...
  },
  {
    ...
    pricingModel: 'custom',
    splitBreakdown: [
      { label: 'Enterprise onboarding', description: 'Dedicated success team + custom integrations', agentKeepPct: 52, platformPct: 48 },
      { label: 'Certified partner', description: 'Custom agreement — you run the org', agentKeepPct: 65, platformPct: 35 },
      { label: 'Volume ceiling', description: 'Negotiated for multi-market operators', agentKeepPct: 68, platformPct: 32 },
    ],
    platformShareMinPct: 32,
    platformShareMaxPct: 48,
    whiteLabelLevel: 'enterprise_white_label',
    recommendedTrainingPhase: 'partner',
    features: [
...
/** Resolve split rows — falls back to legacy min/max platform % if needed. */
export function getAgencyTierSplitBreakdown(tier: AgencyTier): AgencyTierSplitRow[] {
  if (tier.splitBreakdown?.length) return tier.splitBreakdown;
  if (tier.platformShareMinPct == null || tier.platformShareMaxPct == null) return [];
  return [
    {
      label: 'While training',
      agentKeepPct: 100 - tier.platformShareMaxPct,
      platformPct: tier.platformShareMaxPct,
    },
    {
      label: 'When certified',
      agentKeepPct: 100 - tier.platformShareMinPct,
      platformPct: tier.platformShareMinPct,
    },
  ];
}

/** Headline keep range from first → last breakdown row (excludes optional ceiling rows for custom tiers). */
export function getAgencyTierKeepRange(tier: AgencyTier): { min: number; max: number } | null {
  const rows = getAgencyTierSplitBreakdown(tier);
  if (!rows.length) return null;
  const keeps = rows.map((r) => r.agentKeepPct);
  return { min: Math.min(...keeps), max: Math.max(...keeps) };
}

export function formatAgencyTierKeepHeadline(tier: AgencyTier): string | null {
  const rows = getAgencyTierSplitBreakdown(tier);
  if (!rows.length) return null;
  const keeps = rows.map((r) => r.agentKeepPct);
  const min = Math.min(...keeps);
  const max = Math.max(...keeps);
  if (tier.pricingModel === 'custom') {
    const certified = rows.find((r) => r.label.toLowerCase().includes('certified')) ?? rows[1];
    return `From ${certified?.agentKeepPct ?? min}% your keep`;
  }
  if (min === max) return `${min}% your keep`;
  return `${min}–${max}% your keep`;
}
