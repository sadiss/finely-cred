...
export function signupLegalPreviewText(id: SignupLegalItemId, role: OnboardingRole): string {
  ...
  switch (id) {
    ...
    case 'servicesAgreement':
      if (role === 'agent') {
        return 'Independent partner terms: revenue share (not employment), training access, white-label rules, customer file handling, and compliance expectations while active.';
      }
      if (role === 'affiliate') {
    ...
    case 'partnershipExit':
      if (role === 'agent') {
        return 'If you stop operating as a specialist: export customer lists you own, finish in-flight disputes per your agreement, revoke white-label assets, and settle revenue share through the final payout cycle. No non-compete solicitation — standard confidentiality and data return apply.';
      }
