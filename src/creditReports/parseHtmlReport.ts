...
function parsePaymentHistoryFromCodesString(byBureauText: Partial<Record<Bureau, string>>): PaymentHistory2Y | undefined {
  const tokensFrom = (raw: string) => {
    ...
    // Remove obvious month labels that sometimes appear inline.
    const scrubbed = s
    ...
    // Only keep short code-like tokens to avoid capturing balances/dates.
  ...
  // We often can't reconstruct months/years from this representation; UI will still render a usable grid.
  return { months: [], years: [], byBureau };
}
...
function enrichTradeline(t: ParsedTradeline): ParsedTradeline {
  ...
  // Payment history fallback (provider-dependent): sometimes present as a single field row rather than a dedicated grid table.
...
function parseCreditReportHtmlDocument(doc: Document, provider: CreditReportProvider): ParsedCreditReport {
  ...
  const enrichedTradelines = deduped.map(enrichTradeline);
